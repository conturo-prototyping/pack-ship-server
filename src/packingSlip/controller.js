...
const PackingSlip = require('./model');
const { LogError, ExpressHandler, HTTPError } = require('../utils');
const { GetOrderFulfillmentInfo } = require('../shopQ/controller');
 ...
 * @param {mongoose.Schema.Types.ObjectId?} matchId Only populate the specified packingSlipId
 */
...
/**
 * Get the selected Packing Slip as a PDF document definition.
 * The doc def will be rendered in the front-end with pdfmake library.
 */
function getAsPDF(req, res) {
  ExpressHandler(
    async () => {
      ...
      const fulfilledBlock = pdfmakeFulfilledBlock(
        packingSlips.map((x) => x.items).flat(),
        shopQOrderInfo.itemsInOrder,
      );

      const data = pdfMakeDocDef(
...
async function searchPackingSlips(req, res) {
  ExpressHandler(
    async () => {
      const { customer, shipment } = req.query;
...
async function searchHistPackingSlips(req, res) {
  ExpressHandler(
    async () => {
      ...
      if (sortBy !== 'ORDER' && sortBy !== 'DATE' && sortBy !== 'SLIPNUM') sortBy = 'DATE';
      if (sortOrder === '-1' || sortOrder === '1') {
        sortOrder = parseInt(sortOrder, 10);
      } else {
        sortOrder = 1;
      }
      if (Number.isNaN(+pageNumber) || pageNumber < 0) pageNumber = 0;
...
async function createPackingSlip(req, res) {
  ExpressHandler(
    async () => {
      const {
        items, orderNumber, customer, destination,
      } = req.body;
...
async function mergePackingSlips(req, res) {
  ExpressHandler(
    async () => {
      ...
      // fix qties to not have a bunch of packing slips
      // with duplicate itemIds & qties all over the place
      const items = [];
...
function pdfMakeDocDef(packingSlipDoc, fulfilledBlock, shopQOrderInfo) {
  const {
    orderNumber, packingSlipId, items, dateCreated, createdBy,
  } = packingSlipDoc;
  const { shippingContact, purchaseOrderNumber } = shopQOrderInfo;
  const customerTitle = packingSlipDoc.customer.title;

  const bannerBlock = pdfmakeBannerBlock(
    orderNumber,
    dateCreated,
    purchaseOrderNumber,
  );
  const shipToBlock = pdfmakeShipToBlock(customerTitle, shippingContact);
  const manifestBlock = pdfmakeManifestBlock(items, 'Items In Package');
  const signaturesBlock = pdfmakeSignaturesBlock(createdBy);
 ...
 */
function pdfmakeBannerBlock(orderNumber, dateCreated, purchaseOrderNumber) {
  const table = {
    widths: ['auto', '*', 'auto'],
    body: [
      [
        { rowSpan: 4, image: pdfGetLogoURI(), width: 200 },
        {
          colSpan: 2,
          text: 'PACKING SLIP',
          bold: true,
 ...
 * @returns
 */
function pdfmakeShipToBlock(customerTitle, shippingContact) {
 ...
 */
function pdfmakeManifestBlock(items, tableTitle) {
  const body = [
    [
      {
        colSpan: 4,
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const {
      partNumber, partDescription, partRev, quantity,
    } = item;
 ...
 */
function pdfmakeSignaturesBlock(packedByUsername) {
  return {
    table: {
      widths: ['auto', '*'],
      body: [
 ...
 *
 */
function pdfmakeFulfilledBlock(allPackingSlipItems) {
