...
router.post('/stuff', async (req, res) => {
  const agg = [
    ...
    { $addFields: {
      // _partRouter: { $cond: [
      //   // { $ne: ['$workOrder.Items.partRouter', undefined] },
      //   { '$workOrder.Items.partRouter': { $exists: true } },
      //   '$workOrder.Items.partRouter',
      //   ['default']
      // ] },
      // _partRouter: '$workOrder.Items.partRouter',
  ...
  const a = await PackingSlip.aggregate(agg);

  //check for misalignment - the only issues were UATC to AUR
  // a.forEach( x => {
  //   if ( x.packingSlipId.split('-')[0] !== x.orderNumber ) console.log(x)
  // })
