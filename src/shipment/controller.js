...
async function getAsPdf(req, res) {
  ExpressHandler(
    async () => {
      const {
        manifest, customer, dateCreated, shipmentId, deliveryMethod,
      } = req.body;
      ...
      for (const orderNumber of orderNumbers) {
        ...
        for (const ps of packingSlips) {
          ...
          for (const i of items) {
            ...
            if (_id in report === false) {
              report[_id] = item;
              report[_id].qtyShipped = qty;
            } else {
              report[_id].qtyShipped += qty;
            }
          }
        ...
        manifestBlocks.push(pdfMakeManifestBlock(items, tableTitleArr, pageBreakAfter));
        idx += 1;
      }
      ...
      const shippingBlock = pdfMakePackingBlock(title, allShipmentsInfo[0].shippingContact);
      const bannerBlock = pdfMakeBannerBlock(new Date(dateCreated));

      const signatureBlock = pdfMakeSignaturesBlock(deliveryMethod);
...
function pdfMakeBannerBlock(dateCreated) {
  const table = {
    widths: ['auto', '*', 'auto'],
    body: [
      [
        { rowSpan: 2, image: pdfGetLogoURI(), width: 200 },
        {
          colSpan: 2,
          text: 'SHIPMENT',
          bold: true,
...
function pdfMakePackingBlock(customerTitle, shippingContact) {
 ...
 */
function pdfMakeSignaturesBlock(deliveryMethod) {
  const ret = {
    table: {
      widths: ['*', '*'],
    },
 ...
 */
function pdfMakeManifestBlock(items, tableTitleArr, pageBreak) {
  const body = [
    [
      {
        colSpan: 2,
  ...
  for (const i of items) {
    ...
    const {
      partNumber, partDescription, partRev, quantity, qtyShipped,
    } = i;
    ...
    if (partDescription && partDescription?.trim() !== '-') lineText += `\n ${partDescription}`;
