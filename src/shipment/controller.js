...
async function editOne(req, res) {
  ExpressHandler(
    async () => {
      ...
      if ( !isDueBack || isDueBack && isDueBackOn ) {

        const query = {
          sourceShipmentId: sid,
          receivedOn: { $exists: false }, 
        };
        const incomingDeliveries = await IncomingDelivery.find(query);

        let deliveryIds = [];
        const query2 = { _id: { $in: deliveryIds } }

        if ( !isDueBack ) {
          deliveryIds = incomingDeliveries.map( x => x._id );
          IncomingDelivery.deleteMany( query2 );
        }
        ...
        if ( isDueBack && isDueBackOn ) {
          if ( incomingDeliveries.length === 0 ) {
            ...
            const newIncomingDelivery = new IncomingDelivery(_newIncomingDelivery);
            newIncomingDelivery.save();
          }
          else {
            //modify incomingDelivery
            if ( !deliveryIds.length ) deliveryIds = incomingDeliveries.map( x => x._id );
 ...
 */
async function getPO_NumberFromShipmentId(sid) {
  try {
    const agg = [
      { $match: {
        $expr: {
