      ...
      for (m of shipment.manifest) {
        for (item of m.items) {
          ...
          shipmentDollarValue = currency(shipmentDollarValue)
            .add(currency(finalUnitRate).multiply(qty))
            .format();
        }
      }
    ...
    async () => {
      ...
      // if it isDueBack and isDueBackOn then we want to either create a new incomingDelivery or modify existing ones
        ...
        if (incomingDeliveries.length === 0) {
          ...
          const { _id } = req.user;
          const [err, internalPurchaseOrderNumber] =
            await getPoNumberFromShipmentId(sid);
                      ...
                      pipeline: [
                        ...
                        { $unwind: "$content.order.items" },
                        {
                          $match: {
                            $expr: {
                              $eq: [
                                { $toString: "$content.order.items._id" },
                                "$$calcItemId",
                              ],
                            },
                          },
                        },
                        {
                          $addFields: {
                            finalUnitRate:
                              "$content.order.items.calculations.header.finalUnitRate",
                          },
                        },
                        {
                          $project: {
              ...
              $group: {
                ...
                items: {
                  $push: {
                    ...
                    finalUnitRate: {
                      $arrayElemAt: ["$items.finalUnitRate", 0],
                    },
                  },
                },
                customer: { $first: "$customer" },
